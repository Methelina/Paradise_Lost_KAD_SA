# AmuleD v0.4.1 Roadmap

Весь диагностический вывод переведён на обязательную tagged logging схему: стабильные модульные теги `APP`, `CLI`, `CONFIG`, `STATE`, `IMPORT`, `SERVER`, `KAD`, `ED2K`, `SEARCH`, `DOWNLOAD`, `UPLOAD`, `PEER`, `SHARE`, `HASH`, `CODEC`, `SECURITY`, `IPFILTER`, `NAT`, `DAEMON`, `INSTALL`, `RUNNER` и `TEST`. Python пишет tagged JSONL в `AmuleD_v2\logs\amuled.jsonl`; PowerShell использует `[INSTALL] [LEVEL]` и `[RUNNER] [LEVEL]`. Это позволит будущему GUI раскладывать логи по отдельным окнам, а скриптам — разбирать поток по модулю. Дата фиксации: 2026-09-22. Публичное имя клиента зафиксировано как `AmuleD v0.4.1`; технические имена `amuled_v2`, `amuled` и `AmuleD_v2` сохраняются для package, executable и project root. Текущий статус: M0–M3 завершены; M4 TCP login подтверждён на живом сервере. Готовы MD4/ED2K/SHA1/AICH hashing, binary codec, tags, packet framing, zlib, tagged logging, ED2K `OP_LOGINREQUEST`, async ED2K server TCP session, `OP_SERVERIDENT`, `OP_SERVERSTATUS`, `OP_SERVERMESSAGE` и полный eMule-compatible `OP_IDCHANGE`, включая server flags, primary TCP port и reported IP. Живой handshake против `176.123.5.89:4725` выполнен успешно; сервер присвоил `client_id=9382172` и сообщил lowid. Ещё не реализованы live server search, source request, KAD bootstrap и обход lowid через NAT/UPnP. Базовые v1 ресурсы входят в `AmuleD_v2\assets\v1`, runtime и installer не требуют донорной папки. Полный тестовый набор — 128 passed. Этот документ является исходной продуктово-технической спецификацией для полностью чистого Python-клиента ED2K/Kademlia в папке [AmuleD_v2](file:///O:/Work/Coding/Paradise_Lost_KAD_SA/AmuleD_v2). Скрипты [AmuleD_install.ps1](file:///O:/Work/Coding/Paradise_Lost_KAD_SA/AmuleD_v2/AmuleD_install.ps1), [AmuleD_Run.ps1](file:///O:/Work/Coding/Paradise_Lost_KAD_SA/AmuleD_v2/AmuleD_Run.ps1) и [requirements.txt](file:///O:/Work/Coding/Paradise_Lost_KAD_SA/AmuleD_v2/requirements.txt) сохраняют авторскую структуру приветствия/меню и реализуют полностью портативный Trellis2-style uv runtime.
...
### M3 — hash и binary codec layer — DONE
...
ED2K TCP login подтверждён живым сервером `176.123.5.89:4725`: получены `OP_SERVERMESSAGE`, `OP_IDCHANGE`, `client_id`, server flags, primary TCP port и reported IP. Сервер сообщил lowid, поэтому далее нужны source/search слои и отдельно NAT/UPnP диагностика. Реализуются server search, `OP_GETSOURCES`, reconnect и server statistics поверх async session. Основные источники: [Client2Server\TCP.h](file:///O:/Work/Coding/aMule-2.3.3/src/include/protocol/ed2k/Client2Server/TCP.h), [Client2Server\UDP.h](file:///O:/Work/Coding/aMule-2.3.3/src/include/protocol/ed2k/Client2Server/UDP.h), [ServerList.cpp](file:///O:/Work/Coding/aMule-2.3.3/src/ServerList.cpp), [ServerConnect.cpp](file:///O:/Work/Coding/aMule-2.3.3/src/ServerConnect.cpp), [SearchList.cpp](file:///O:/Work/Coding/aMule-2.3.3/src/SearchList.cpp) и eMule 0.50a [ServerSocket.cpp](file:///O:/Work/Coding/eMule_0.50a/srchybrid/ServerSocket.cpp).
...
Следующий рабочий шаг — реализовать ED2K server search и `OP_GETSOURCES` поверх async TCP session, добавить response dispatch и source persistence. Затем подключается KAD bootstrap и unified search.
